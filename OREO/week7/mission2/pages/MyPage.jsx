...
export default function MyPage() {
  ...
  // ⭐ 낙관적 업데이트 (mission 2의 핵심)
  // - onMutate: 변경 직전 user 스냅샷을 context로 들고, AuthContext의 user를 즉시 갱신.
  //             → Nav-Bar의 닉네임이 서버 응답을 기다리지 않고 즉시 변경됨.
  // - onError: 스냅샷으로 정확히 롤백 (단순 invalidate가 아니라 prev 값으로 set).
  // - onSettled: 어떤 결과든 마지막에 me 캐시를 무효화해 서버 진실과 동기화.
  const updateMut = useMutation({
    mutationFn: updateMe,
    onMutate: async (patch) => {
      await qc.cancelQueries({ queryKey: qk.users.me() })
      const prevUser = user
      const prevMe = qc.getQueryData(qk.users.me())
      // 화면 즉시 갱신 — Nav-Bar(헤더) + MyPage 모두 동기화
      patchUser(patch)
      qc.setQueryData(qk.users.me(), (old) =>
        old ? { ...old, ...patch } : old,
      )
      return { prevUser, prevMe }
    },
    onError: (_err, _patch, ctx) => {
      if (ctx?.prevUser) patchUser(() => ctx.prevUser)
      if (ctx?.prevMe !== undefined) qc.setQueryData(qk.users.me(), ctx.prevMe)
    },
    onSettled: () => {
