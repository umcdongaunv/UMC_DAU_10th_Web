...
export const LpDetailPage = () => {
  ...
  const isLiked = lpRes?.data ? lpRes.data.likes.length > 0: false;
    ...
    <div className="min-h-screen bg-black flex flex-col items-center p-4 mt-18 gap-8">
      ...
      <article className="w-full max-w-2xl bg-[#161b22] rounded-3xl border border-neutral-800 p-8 shadow-2xl">
        ...
        <div className="pt-6 border-t border-neutral-800 flex justify-center">
          <button onClick={handleLikeToggle} className="flex items-center gap-2 group active:scale-95 transition-transform">
            <Heart size={28} className={isLiked ? "text-rose-500" : "text-neutral-500"} fill={isLiked ? "currentColor" : "none"} />
            <span className={`text-xl font-bold ${isLiked ? "text-rose-500" : "text-neutral-500"}`}>{lp.likes.length}</span>
          </button>
        </div>
      </article>
